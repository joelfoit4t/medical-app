...
const MaterialTimePicker: React.FC<{
  ...
  return (
    <div className="absolute top-full right-0 mt-2 bg-white rounded-3xl shadow-[0_20px_50px_-10px_rgba(0,0,0,0.2)] border border-slate-100 z-[160] w-[240px] overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right">
...
export const AddAppointmentView: React.FC<Props> = ({ onAddAppointment, onSuccess }) => {
  ...
  const handleReset = () => {
    setFormData({ patientName: '', reason: '', date: '', startTime: '', endTime: '', notes: '' });
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
            <div className="space-y-2.5">
              ...
              <div className="relative group">
                ...
                <input 
                  type="text" required placeholder="Select or enter patient name"
            ...
            <div className="space-y-2.5">
              ...
              <div className="relative group">
                ...
                <input 
                  type="text" placeholder="e.g. Annual Check-up"
                  className="w-full pl-14 pr-5 py-4 bg-[#f8fafc] border border-slate-200 rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all placeholder:text-slate-300 font-bold text-slate-700 h-[56px]"
            ...
            <div className="space-y-2.5 relative" ref={datePickerRef}>
              ...
              {showDatePicker && (
                <MaterialDatePicker value={formData.date} onChange={(val) => setFormData({...formData, date: val})} onClose={() => setShowDatePicker(false)} />
              )}
            </div>

            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-2.5 relative" ref={startTimeRef}>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Start Time</label>
                <div 
                  onClick={() => setShowStartTimePicker(!showStartTimePicker)}
                  className="w-full px-5 py-4 bg-[#f8fafc] border border-slate-200 rounded-2xl text-sm h-[56px] transition-all font-bold text-slate-700 flex items-center justify-between cursor-pointer hover:bg-white group"
                >
                  <span className={formData.startTime ? 'text-slate-700' : 'text-slate-300'}>
                    {formatTimeDisplay(formData.startTime)}
                  </span>
                  <div className="flex items-center gap-1.5">
                    <Clock className="text-slate-300 group-hover:text-emerald-600 transition-colors" size={18} />
                    <ChevronDown className={`text-slate-200 transition-transform ${showStartTimePicker ? 'rotate-180' : ''}`} size={14} />
                  </div>
                </div>
                {showStartTimePicker && (
                  <MaterialTimePicker value={formData.startTime} onChange={(val) => setFormData({...formData, startTime: val})} onClose={() => setShowStartTimePicker(false)} />
                )}
              </div>

              <div className="space-y-2.5 relative" ref={endTimeRef}>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">End Time</label>
                <div 
                  onClick={() => setShowEndTimePicker(!showEndTimePicker)}
                  className="w-full px-5 py-4 bg-[#f8fafc] border border-slate-200 rounded-2xl text-sm h-[56px] transition-all font-bold text-slate-700 flex items-center justify-between cursor-pointer hover:bg-white group"
                >
                  <span className={formData.endTime ? 'text-slate-700' : 'text-slate-300'}>
                    {formatTimeDisplay(formData.endTime)}
                  </span>
                  <div className="flex items-center gap-1.5">
                    <Clock className="text-slate-300 group-hover:text-emerald-600 transition-colors" size={18} />
                    <ChevronDown className={`text-slate-200 transition-transform ${showEndTimePicker ? 'rotate-180' : ''}`} size={14} />
                  </div>
                </div>
                {showEndTimePicker && (
                  <MaterialTimePicker value={formData.endTime} onChange={(val) => setFormData({...formData, endTime: val})} onClose={() => setShowEndTimePicker(false)} />
                )}
              </div>
            </div>
            ...
            <div className="space-y-2.5 md:col-span-2">
              ...
              <textarea 
                rows={5} placeholder="Additional instructions for the doctor..."
